...
public extension BaseDirectories {
	...
	func ensureConfigDirPath(_ path: FilePath) throws -> FilePath {
		return try configHomePrefixed.lexicallyResolving(path).ensureExistingDir(with: fileManagerFactory())
	}
	...
	func ensureDataDirPath(_ path: FilePath) throws -> FilePath {
		return try dataHomePrefixed.lexicallyResolving(path).ensureExistingDir(with: fileManagerFactory())
	}
	...
	func ensureCacheDirPath(_ path: FilePath) throws -> FilePath {
		return try cacheHomePrefixed.lexicallyResolving(path).ensureExistingDir(with: fileManagerFactory())
	}
	...
	func ensureStateDirPath(_ path: FilePath) throws -> FilePath {
		return try stateHomePrefixed.lexicallyResolving(path).ensureExistingDir(with: fileManagerFactory())
	}
	...
	func ensureRuntimeDirPath(_ path: FilePath) throws -> FilePath {
		return try runtimeDirPrefixed.get().lexicallyResolving(path).ensureExistingDir(with: fileManagerFactory())
	}
