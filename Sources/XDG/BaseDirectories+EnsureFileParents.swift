...
public extension BaseDirectories {
	...
	func ensureParentsForConfigFilePath(_ path: FilePath) throws -> FilePath {
		return try configHomePrefixed.lexicallyResolving(path).ensureExistingParent(with: fileManagerFactory())
	}
	...
	func ensureParentsForDataFilePath(_ path: FilePath) throws -> FilePath {
		return try dataHomePrefixed.lexicallyResolving(path).ensureExistingParent(with: fileManagerFactory())
	}
	...
	func ensureParentsForCacheFilePath(_ path: FilePath) throws -> FilePath {
		return try cacheHomePrefixed.lexicallyResolving(path).ensureExistingParent(with: fileManagerFactory())
	}
	...
	func ensureParentsForStateFilePath(_ path: FilePath) throws -> FilePath {
		return try stateHomePrefixed.lexicallyResolving(path).ensureExistingParent(with: fileManagerFactory())
	}
	...
	func ensureParentsForRuntimeFilePath(_ path: FilePath) throws -> FilePath {
		return try runtimeDirPrefixed.get().lexicallyResolving(path).ensureExistingParent(with: fileManagerFactory())
	}
